...
def do_sql(post):
    ...
    cursor = conn.cursor()
    try:
        sql = "select * from spider where article_id = %s"
        data = post["id"]
        cursor.execute(sql, data)
        data = cursor.fetchall()
        if len(data) == 0:
            # 执行插入操作
            sql = "INSERT INTO spider (article_id, article_title, view_count, website_url, article_time) VALUES (%s, %s, %s, %s, %s)"
            data = (post["id"], post["title"], post["view_count"], 'upbit', post["created_at"])
            cursor.execute(sql, data)
            # 提交事务
            conn.commit()
            return True
